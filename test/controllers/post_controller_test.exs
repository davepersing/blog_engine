defmodule BlogEngine.PostControllerTest do
  ...
  setup do
    ...
    admin = Factory.create(:user, role: admin_role)

    conn = conn() |> login_user(user)
    {:ok, conn: conn, user: user, other_user: other_user, admin: admin, role: role, post: post}
  end
  ...
  test "when logged in as the author, shows chosen resource with author flag set to true", %{conn: conn, user: user, admin: admin} do
